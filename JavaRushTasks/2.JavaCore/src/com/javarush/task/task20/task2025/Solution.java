...
public class Solution {
    ...
    /*public static long[] getNumbers(long N) {
        ...
        for (int i = 0; i < N; i++) {
            long ch = i;
            int c = countDigits_B(N);
            if(ch==getNumberSumPowerch(i,c)) {
                result[index] = getNumberSumPowerch(i,c);
                index++;
            }
        }
        Arrays.sort(result);
    ...
    static int[] getArrInteger(long ch, long c){
        int[] res = new int[(int)c];
        for (int i = 0; i < c; i++) {
            if ((ch%10)==0){
                res[(int)c-i-1] = 0;
                ch = ch/10;
    ...
    static long getPower(int ch, int c){
        long rez=1;
        for (int i = 0; i < c; i++) {
            rez=rez*ch;
        }
    ...
    static long getNumberSumPowerch(int ch, int c){
        int[] arr = null;
        long d = 0;
        arr = getArrInteger(ch,c);
        for (int j = 0; j < arr.length; j++){
            d=d+getPower(arr[j],c);
        }
        return d;
    }
